...
export function compareStringToProps(
  str: string,
  props,
  debug = false
): number {
  ...
  matchAllResults.forEach((item) => {
    ...
    if (!propValue) {
      ...
      debug &&
        console.log(
          `property check false(prop not exists or empty) for: ${propName}, while serve ${item[0]}`
        );
      ...
      switch (propCompare) {
        ...
        case '<=':
          ...
          debug &&
            console.log(
              `propValue = ${parseFloat(propValue)} ; propTest = ${parseFloat(
                propTest
              )}; compare = ${propCompare} ; result = ${result}`
            );
          break;
        case '>=':
          result = parseFloat(propValue) > parseFloat(propTest);
        ...
        default:
          ...
          debug &&
            console.log(
              `compare string error. wrong part: ${matchProp[0]} at the moment: ${str}`
            );
          str = '-1';
      }
      ...
      switch (compare) {
        case '':
          switch (inner) {
            ...
            default:
              console.log(
                `compare string error. wrong part: ${inner} at the moment: ${str}`
              );
              str = '-1';
          }
          break;
...
export function getContractAddress(
  ...
  const contractNode = getXMLItem(
    xmlDoc,
    `/ts:token/ts:contract[@name="${ethContract}"][1]`
  );
  let contractInterface;
  let contractAddress;
  if (contractNode) {
    contractInterface = contractNode.getAttribute('interface');
    contractAddress = getXMLItemText(
      xmlDoc,
      `ts:address[@network='${chainID}'][1]`,
      contractNode
    );
  }
...
export function getXMLItemText(
  xmlDoc,
  selector,
  context = '',
  fallbackSelector = '',
...
export async function getJSONAbi(
  ethContract,
  jsons,
  path = '',
  debug = 0,
...
export function getEthereumCallParams({
  userAddress,
  ethereumNode,
  xmlDoc,
  tokenName,
  ...
  ethCallAttributtes.contract = ethCallAttributtes.contract
...
export function getErc20EventParams(erc20EventName, xmlDoc): {} {
  ...
  if (moduleNode) {
    const eventElementsXmlNodes = xmlDoc.evaluate(
...
export function getXMLViews(xmlDoc): any {
  ...
  const cardNodes = xmlDoc.evaluate(
    '/ts:token/ts:cards/ts:card',
    xmlDoc,
    nsResolver,
    XPathResult.ANY_TYPE,
    null
  ...
  while ((cardNode = cardNodes.iterateNext())) {
    ...
    const viewNodes = xmlDoc.evaluate(
      'ts:item-view|ts:view',
      cardNode,
      nsResolver,
      XPathResult.ANY_TYPE,
      null
...
export function extendPropsWithContracts(
  xmlDoc,
  props = {},
  ...
  const contractNodes = xmlDoc.evaluate(
    '/ts:token/ts:contract',
    xmlDoc,
    nsResolver,
    XPathResult.ANY_TYPE,
    null
  ...
  while ((contractNode = contractNodes.iterateNext())) {
    ...
    const address = getXMLItemText(
      xmlDoc,
      `ts:address[@network="${ethersData.chainID}"]`,
      contractNode
    );
    if (address) {
      output[name] = address;
...
export class TokenCard {
  ...
  constructor(data) {
    ...
    this.nsResolver = this.xmlDoc.createNSResolver(
    ...
    this.defaultContract = this.defaultContract
  ...
  async getDistinctItems(): Promise<{ distinctName: string; items: [string] }> {
    const distinctAttributeNode = getXMLItem(
      this.xmlDoc,
      '/ts:token/ts:attribute[@distinct="true"][1]'
    );
    ...
    if (!distinctAttributeNode) {
      ...
      const [attrError, distinctData] = await to(
        this.getAttributeValue(distinctName)
      );
      const items = [];
      if (attrError) {
  ...
  async getProps(): Promise<any> {
    ...
    try {
      ...
      const attributeXmlNodes = this.xmlDoc.evaluate(
        '/ts:token/ts:attribute',
        this.xmlDoc,
        this.nsResolver,
        XPathResult.ANY_TYPE,
        null
  ...
  async getAttributeValue(name): Promise<any> {
    ...
    const originsInnerNode = getXMLItem(
      this.xmlDoc,
      `/ts:token/ts:attribute[@name="${name}"]/ts:origins/*[1]`
    );
    if (!originsInnerNode) {
      return null;
    }
    ...
    switch (dataNodeName) {
      ...
      case 'ethereum:call':
        ...
        if (!abi) {
          console.error(
            `Cant get Contract ABI for "${ethCallAttributtes.contract}" , attribute "${name}" skipped`
          );
        ...
        const [error, output] = await to(
          ethersContract[ethCallAttributtes.function].apply(null, params)
        );
        if (error) {
  ...
  returnCardFromIframe(props, listener$, iframe, iframeId): void {
    ...
    if (iframe.contentWindow.web3.tokens.dataChanged) {
      ...
      iframe.contentWindow.web3.tokens.dataChanged(
        null,
        { currentInstance: propsForRender },
        'render'
      );
    }
  ...
  async getCardItems({
    ...
    const originsNodes = this.xmlDoc.evaluate(
      selector,
      cardNode,
      this.nsResolver,
      XPathResult.ANY_TYPE,
    ...
    if (originsNodes && cardType === 'activity') {
      ...
      while (originsNodes && (ethereumEventNode = originsNodes.iterateNext())) {
        ...
        // tslint:disable-next-line:prefer-const
        let { contractInterface, contractAddress } = getContractAddress(
          this.xmlDoc,
          ethContract,
          this.ethersData.chainID
        );
        ...
        if (!ethContract || !contractAddress) {
          throw new Error(
            ` ethContract and contractAddress required for {Address=${this.ethersData.userAddress}, chainID = ${this.ethersData.chainID}, tokenName = ${this.tokenName}. Check logs for details`
          );
        }
        ...
        if (!params) {
          throw new Error(eventType + ' parameters missing.');
        }
        ...
        if (!abi) {
          throw new Error(`Empty ABI for ${ethContract}`);
        }
        ...
        if (returnHistory) {
          ...
          if (contractError) {
            throw new Error(
              `Error when try to request ${eventType}: ${contractError}`
            );
          }
